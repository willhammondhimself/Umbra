...
final class DatabaseManager {
    ...
    func fetchTasks(projectId: Int64? = nil) throws -> [UmbraTask] {
        try dbQueue.read { db in
    ...
    func saveTask(_ task: inout UmbraTask) throws {
        task.updatedAt = Date()
        try dbQueue.write { db in
            try task.save(db)
        }
    ...
    func deleteTask(_ task: UmbraTask) throws {
        try dbQueue.write { db in
            _ = try task.delete(db)
        }
    }

    func updateTaskOrder(taskId: Int64, newOrder: Int) throws {
        try dbQueue.write { db in
            try db.execute(
    ...
    func nextSortOrder(projectId: Int64?) throws -> Int {
        try dbQueue.read { db in
    ...
    func saveSession(_ session: inout Session) throws {
        try dbQueue.write { db in
            try session.save(db)
        }
    }

    func fetchSessions(limit: Int = 50) throws -> [Session] {
        try dbQueue.read { db in
            try Session
    ...
    func fetchIncompleteSession() throws -> Session? {
        try dbQueue.read { db in
            try Session
    ...
    func saveEvent(_ event: inout SessionEvent) throws {
        try dbQueue.write { db in
            try event.save(db)
        }
    }

    func fetchEvents(sessionId: Int64) throws -> [SessionEvent] {
        try dbQueue.read { db in
            try SessionEvent
    ...
    func fetchBlocklistItems() throws -> [BlocklistItem] {
        try dbQueue.read { db in
            try BlocklistItem.order(Column("displayName").asc).fetchAll(db)
        }
    }

    func saveBlocklistItem(_ item: inout BlocklistItem) throws {
        try dbQueue.write { db in
            try item.save(db)
        }
    }

    func deleteBlocklistItem(_ item: BlocklistItem) throws {
        try dbQueue.write { db in
    ...
    func upsertProjectFromRemote(remoteId: UUID, name: String, createdAt: Date) throws {
        try dbQueue.write { db in
            ...
            if let existing = try Project
                ...
                .fetchOne(db)
            {
                // Update only if remote is newer (handled by caller)
                try db.execute(
    ...
    func upsertTaskFromRemote(remoteId: UUID, title: String, priority: Int, status: Int, estimateMinutes: Int?, dueDate: Date?, sortOrder: Int, createdAt: Date, updatedAt: Date) throws {
        try dbQueue.write { db in
    ...
    func fetchLastSyncDate(for entity: String) throws -> Date? {
        try dbQueue.read { db in
