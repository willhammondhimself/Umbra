...
final class SessionManager {
    ...
    func startSession() {
        ...
        do {
            ...
            TetherLogger.session.error("Failed to save session: \(error.localizedDescription)")
            return
        }
    ...
    func stopSession() {
        ...
        do {
            ...
            TetherLogger.session.error("Failed to finalize session: \(error.localizedDescription)")
        }
    ...
    private func persistCurrentState() {
        ...
        do {
            ...
            TetherLogger.session.error("Failed to persist state: \(error.localizedDescription)")
        }
    }
    ...
    private func handleAppSwitch(appName: String, bundleId: String) {
        currentApp = appName
        let isTether = bundleId == Bundle.main.bundleIdentifier
        let isExempted = BlockingManager.shared.sessionExemptions.contains(bundleId)

        if !isTether && !isExempted && !appMonitor.isIdle {
    ...
    private func logEvent(_ type: SessionEvent.EventType, appName: String? = nil, duration: Int? = nil) {
        ...
        do {
            ...
            TetherLogger.session.error("Failed to log event: \(error.localizedDescription)")
        }
    }
