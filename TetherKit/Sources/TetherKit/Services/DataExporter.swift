...
public struct DataExporter {
    ...
    public func exportSessions(format: ExportFormat, from startDate: Date, to endDate: Date) throws -> Data {
    ...
    public func exportTasks(format: ExportFormat) throws -> Data {
    ...
    private func tasksToCSV(_ tasks: [TetherTask]) -> Data {
    ...
    private func tasksToJSON(_ tasks: [TetherTask]) throws -> Data {
        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .iso8601
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
