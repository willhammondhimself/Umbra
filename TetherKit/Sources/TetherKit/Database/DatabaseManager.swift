...
public final class DatabaseManager {
    ...
    private init() {
        do {
            ...
            let tetherDir = appSupportURL.appendingPathComponent("Tether", isDirectory: true)
            try fileManager.createDirectory(at: tetherDir, withIntermediateDirectories: true)

            let dbURL = tetherDir.appendingPathComponent("tether.sqlite")
            var config = Configuration()
            #if DEBUG
            config.prepareDatabase { db in
                db.trace { TetherLogger.database.debug("SQL: \($0)") }
            }
            #endif
    ...
    public func fetchTasks(projectId: Int64? = nil) throws -> [TetherTask] {
        try dbQueue.read { db in
            var request = TetherTask.order(
                Column("status").asc,
    ...
    public func saveTask(_ task: inout TetherTask) throws {
        task.updatedAt = Date()
        try dbQueue.write { db in
            try task.save(db)
        }
    ...
    public func deleteTask(_ task: TetherTask) throws {
        try dbQueue.write { db in
    ...
    public func fetchPendingTasks() throws -> [TetherTask] {
        try dbQueue.read { db in
            try TetherTask.filter(Column("syncStatus") != SyncStatus.synced.rawValue).fetchAll(db)
        }
    }
    ...
    public func upsertTaskFromRemote(remoteId: UUID, title: String, priority: Int, status: Int, estimateMinutes: Int?, dueDate: Date?, sortOrder: Int, createdAt: Date, updatedAt: Date) throws {
        try dbQueue.write { db in
            if let existing = try TetherTask
                .filter(Column("remoteId") == remoteId.uuidString)
                .fetchOne(db)
            {
                try db.execute(
                ...
                var task = TetherTask(
                    ...
                    priority: TetherTask.Priority(rawValue: priority) ?? .medium,
                    status: TetherTask.Status(rawValue: status) ?? .todo,
                    dueDate: dueDate,
                    sortOrder: sortOrder,
                    createdAt: createdAt,
                    updatedAt: updatedAt,
