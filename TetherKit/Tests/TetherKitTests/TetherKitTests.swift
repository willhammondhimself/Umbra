...
@Test func testTaskPriorityOrder() {
    #expect(TetherTask.Priority.low < .medium)
    #expect(TetherTask.Priority.medium < .high)
    #expect(TetherTask.Priority.high < .urgent)
}

@Test func testTaskPriorityLabels() {
    #expect(TetherTask.Priority.low.label == "Low")
    #expect(TetherTask.Priority.medium.label == "Medium")
    #expect(TetherTask.Priority.high.label == "High")
    #expect(TetherTask.Priority.urgent.label == "Urgent")
}

@Test func testTaskPriorityIcons() {
    #expect(TetherTask.Priority.low.iconName == "arrow.down")
    #expect(TetherTask.Priority.urgent.iconName == "exclamationmark.2")
}

@Test func testTaskStatusLabels() {
    #expect(TetherTask.Status.todo.label == "To Do")
    #expect(TetherTask.Status.inProgress.label == "In Progress")
    #expect(TetherTask.Status.done.label == "Done")
}

@Test func testTaskStatusAllCases() {
    #expect(TetherTask.Status.allCases.count == 3)
}

@Test func testTaskDefaultValues() {
    let task = TetherTask(title: "Test")
    #expect(task.id == nil)
...
@Test func testFormattedEstimate() {
    var task = TetherTask(title: "Test")
...
@Test func testDatabaseTaskCRUD() throws {
    ...
    var task = TetherTask(title: "Write tests")
...
@Test func testTaskCodableRoundtrip() throws {
    let original = TetherTask(title: "Test", estimateMinutes: 60, priority: .high, status: .inProgress)
    ...
    let decoder = JSONDecoder()
    decoder.dateDecodingStrategy = .iso8601
    let decoded = try decoder.decode(TetherTask.self, from: data)
